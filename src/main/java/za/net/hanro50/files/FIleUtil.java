...
import za.net.hanro50.types.FileUtilConfig;

@Log_Ignore()
public class FIleUtil {
	/**
	 * The Gson object used to generate and read json data. See
	 * {@link za.net.hanro50.files.JsonObj JsonObj} or {@link za.net.hanro50.files.ConfigObj ConfigObj}
	 */
	...
	/**
	 * Checks if a file exists within the {@link #DefaultClassPath ClassPath} folder. </br>
	 * 
	 * @see #FileChkroot(String) FileChkroot(String Path) for a version of
	 ...
	 * 
	 * @implNote Does not check more then one level. If you want to check "F/C/".
	...
	public void FileChkroot(String Path) {
		try {
			...
			Log.pek("Checking: " + F.getAbsolutePath());
			if (!F.exists())
				F.mkdir();
		} catch (Throwable e) {
			Log.trace(e);
	 ...
	 * @param extension The extension of the files you're looking for
	 * @return A list of file objects
	 * @see za.net.hanro50.files.DirObj DirObj
	 */
	public File[] listFiles(String path, String extension) {
		DirObj F = new DirObj(path, this);
		return F.listFiles(extension);
	}
	...
	public String[] FileListString(String path, String extension) {
		extension = SafeStr(extension);
		File[] FL = listFiles(path, extension);
	...
	public FileObj getFileObj(String ClassPath, String Path, String FileName, String FileExtension) {
		return new FileObj(ClassPath, Path, FileName, FileExtension, this);
	}

	public FileObj getFileObj(String Path, String FileName, String FileExtension) {
		return new FileObj(Path, FileName, FileExtension, this);
	}
	 ...
	 * @author <a
	 *         href=https://stackoverflow.com/questions/3987921/not-able-to-delete-the-directory-through-java>
	 *         Riduidel</a>
	 */
	...
	public static boolean deletePath(File file) {
		if (file.exists() && file.isDirectory()) {
			...
			for (File value : files) {
				if (value.isDirectory()) deletePath(value); else value.delete();
			}
		}
		return (file.delete());
	}
